...
/**
 * @brief draw a circle with the given origin and radius on the screen.
 *
 * @param origin: the center coordiante of the circle on the screen
 * @param radius: radius of the circle
 */
void rst::rasterizer::draw_circle(const Eigen::Vector2i& origin, int radius) {
    Eigen::Vector3f line_color = {255, 255, 255};
    int x = 0;
    int y = radius;
    int p = 3 - 2 * radius;

    auto draw_octant_points = [&](int X, int Y) {
        // Draw points in all eight octants based on symmetry
        frame_buf[get_index(X + origin.x(), Y + origin.y())] = line_color;
        frame_buf[get_index(Y + origin.x(), X + origin.y())] = line_color;
        frame_buf[get_index(-X + origin.x(), Y + origin.y())] = line_color;
        frame_buf[get_index(-Y + origin.x(), X + origin.y())] = line_color;
        frame_buf[get_index(-X + origin.x(), -Y + origin.y())] = line_color;
        frame_buf[get_index(-Y + origin.x(), -X + origin.y())] = line_color;
